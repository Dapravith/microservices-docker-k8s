 ...
 * TEACHER token can never reach student endpoints even if the gateway is bypassed.
 */
@Component
public class JwtAuthFilter extends OncePerRequestFilter {

    private static final String REQUIRED_ROLE = "STUDENT";
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (!REQUIRED_ROLE.equalsIgnoreCase(user.role())) {
            writeError(response, HttpServletResponse.SC_FORBIDDEN, "Forbidden",
                    "role '" + user.role() + "' is not permitted to access student-service");
            return;
        }
